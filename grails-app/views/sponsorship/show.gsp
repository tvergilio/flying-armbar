	...
	<body>
		...
		<div id="show-sponsorship" class="content scaffold-show" role="main">
			...
			<ol class="property-list sponsorship">
				...
				<g:if test="${sponsorshipInstance?.course}">
				<li class="fieldcontain">
					<span id="course-label" class="property-label"><g:message code="sponsorship.course.label" default="Course" /></span>
					
						<span class="property-value" aria-labelledby="course-label"><g:link controller="course" action="show" id="${sponsorshipInstance?.course?.id}">${sponsorshipInstance?.course?.encodeAsHTML()}</g:link></span>
